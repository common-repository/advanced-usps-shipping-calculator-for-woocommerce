<?php
/*
  Plugin Name: USPS Advanced Shipping for WooCommerce
  Plugin URI: http://www.auctioninc.com/info/page/usps_advanced
  Description: Accurate real-time shipping rates from USPS. Includes advanced flat rate box capabilities. Free two week trial. No USPS account required.
  Version: 1.6
  Author: Paid, Inc.
...
function wc_auctioninc_plugin_links($links) {
    ...
    $plugin_links = array(
        '<a href="' . admin_url('admin.php?page=wc-settings&tab=shipping&section=wc_usps_adv_shipping_auctioninc') . '">' . __('Settings', 'wc_auctioninc') . '</a>',
        '<a href="http://auctioninc.helpserve.com">' . __('Support', 'wc_auctioninc') . '</a>',
        '<a href="http://www.auctioninc.com/info/page/usps_advanced">' . __('Docs', 'wc_auctioninc') . '</a>',
    );
...
if (is_woocommerce_active()) {
    ...
    function wc_auctioninc_add_method($methods) {
        $methods[] = 'WC_USPS_ADV_Shipping_AuctionInc';
        return $methods;
    }
    ...
    function wc_auctioninc_add_shipping_fields() {
        ...
        woocommerce_wp_text_input(
                array(
                    'id' => 'auctioninc_lot_size',
                    'label' => __('Lot Size', 'wc_auctioninc'),
                    'placeholder' => '1',
                    'type' => 'number',
                    'custom_attributes' => array(
                        'step' => '1',
                    ...
                    'value' => $lot_size,
                    'desc_tip' => 'true',
                    'description' => __('Number of separate boxes required to ship this item.  When setting lot size > 1, set the weight/dims to the average across the boxes and set Package flag to "Separately".', 'wc_auctioninc')
                )
        );
    ...
    function wc_auctioninc_shipping_fields_save($post_id) {
        ...
        // Lot Size
        $auctioninc_lot_size = isset($_POST['auctioninc_lot_size']) ? $_POST['auctioninc_lot_size'] : 1;
        if (is_numeric($auctioninc_lot_size) && $auctioninc_lot_size > 0) {
            update_post_meta($post_id, 'auctioninc_lot_size', $auctioninc_lot_size);
        }
    ...
    function wc_auctioninc_admin_notice() {
        ...
        if (empty($auctioninc_settings['account_id'])) {
            echo '<div class="error">
             <p>' . __('An') . ' <a href="http://www.auctioninc.com/info/page/usps_advanced" target="_blank">' . __('AuctionInc', 'wc_auctioninc') . '</a> ' . __('account is required to use the USPS Advanced ShippingCalc plugin.  Please enter your AuctionInc Account ID.', 'wc_auctioninc') . '</p>
         </div>';
        }
    }
